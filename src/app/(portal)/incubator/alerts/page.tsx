...
export default function AlertsPage() {
  ...
  useEffect(() => {
    async function load() {
      const [reportsData, enterprisesData] = await Promise.all([
      ...
      <div className="page-container space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: '平均活跃度', value: Math.round(activityReports.reduce((s, r) => s + r.activity_score, 0) / activityReports.length), color: 'text-text-primary', icon: Activity, iconColor: 'text-text-muted' },
          ].map((c, i) => (
