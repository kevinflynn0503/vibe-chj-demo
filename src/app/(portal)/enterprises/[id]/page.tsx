...
import {
  ChevronLeft, ArrowRight, FileText, Video, Building2, Clock, ListChecks,
  Shield, ExternalLink, ChevronRight, CheckCircle2, AlertCircle,
  MapPin, Users, Calendar, TrendingUp, Briefcase, Globe, Rocket,
...
import { DetailSkeleton, Button, Tag } from '@/components/ui';
...
export default function EnterprisePage() {
  ...
  const [visits, setVisits] = useState<Awaited<ReturnType<typeof fetchVisitRecords>>>([]);
  const [demands, setDemands] = useState<Awaited<ReturnType<typeof fetchDemands>>>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const [ent, vis, dem, rep, assess] = await Promise.all([
          fetchEnterprise(id),
        ...
        if (!cancelled) {
          ...
          setDemands(dem ?? []);
          setReport(rep);
          setAssessments((assess ?? []).filter(a => a.enterprise_id === id));
        }
      } finally {
    ...
    return (
      <div className="p-6 max-w-[1200px] mx-auto">
        <div className="flex flex-col items-center py-16">
          <p className="text-sm text-text-secondary">未找到企业 {id}</p>
          <Button size="sm" className="mt-3" onClick={() => router.back()}>返回</Button>
        </div>
      </div>
    );
  }
        ...
        <div className="detail-header-inner">
          ...
          <div className="flex items-center justify-between mb-4">
            <Button variant="ghost" size="sm" onClick={() => router.back()}>
              <ChevronLeft className="h-3.5 w-3.5" /> 返回企业库
            </Button>
            <div className="flex items-center gap-2">
              <Button size="sm" onClick={() => linkMinute(name)} icon={<Video className="h-3.5 w-3.5" />}>
                关联妙记
              </Button>
              <Button size="sm" onClick={() => exportToFeishu(name, '企业画像')} icon={<ExternalLink className="h-3.5 w-3.5" />}>
                导出飞书
              </Button>
              <Button variant="primary" size="sm" onClick={() => generateReport(name)} icon={<FileText className="h-3.5 w-3.5" />}>
                生成背调
              </Button>
            </div>
          </div>
          ...
          <div className="flex gap-6">
            <div className="shrink-0 w-20 h-20 rounded-lg bg-brand text-white flex items-center justify-center text-2xl font-bold border border-brand">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 text-xs text-text-muted mb-3">
                <span className="flex items-center gap-1"><Phone className="h-3 w-3 text-text-muted" /> 021-5423XXXX</span>
                <span className="flex items-center gap-1"><Mail className="h-3 w-3 text-text-muted" /> contact@{enterprise.short_name || 'co'}.com</span>
              ...
              <div className="grid grid-cols-4 gap-4 bg-neutral-50 border border-line-light rounded-lg p-3">
                ...
                <div className="border-l border-line pl-4">
                  <div className="text-tag text-text-muted">注册资本</div>
                  <div className="text-sm font-medium text-text-primary">{enterprise.registered_capital || '-'}</div>
                </div>
                <div className="border-l border-line pl-4">
                  <div className="text-tag text-text-muted">成立日期</div>
                  <div className="text-sm font-medium text-text-primary">{enterprise.established_date || '-'}</div>
                </div>
                <div className="border-l border-line pl-4">
                  <div className="text-tag text-text-muted">员工规模</div>
                  <div className="text-sm font-medium text-text-primary">{enterprise.employee_count || '-'} 人</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Tabs */}
          <div className="mt-4 flex items-center gap-1 overflow-x-auto border-t border-line-light pt-1">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mt-3">
              <Button variant="primary" size="sm" onClick={() => sendChat(`请对「${name}」进行深度综合分析，从市场地位、技术壁垒、财务健康、团队实力、政策机会五个维度出具详细报告。`)}
                icon={<Sparkles className="h-3.5 w-3.5" />}>
                AI 深度分析
              </Button>
              <Button size="sm" onClick={() => sendChat(`请用 Deep Research 搜集「${name}」最新的公开信息、新闻、融资动态、专利申请等。`)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-surface-card border border-line rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-2">
            <TrendingUp className="h-4 w-4 text-success" /> 发展潜力
          </h3>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-start gap-2 text-xs">
                <span className={cn("shrink-0 mt-0.5", item.level === 'positive' ? 'text-success' : 'text-warning')}>
        ...
        <div className="bg-surface-card border border-line rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-2">
            <Shield className="h-4 w-4 text-brand" /> 风险扫描
          </h3>
          <div className="space-y-2">
            {[
        ...
        <div className="bg-surface-card border border-line rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-2">
            <Star className="h-4 w-4 text-warning" /> AI 合作建议
          </h3>
          <div className="space-y-3 text-xs text-text-secondary">
            <p>基于企业画像和园区服务能力，AI 建议：</p>
            <div className="space-y-2">
              <div className="flex items-start gap-2"><span className="text-brand font-bold shrink-0">1.</span> 推荐申请高新技术企业认定（匹配度{assessments.length > 0 ? assessments[0].grade_score : 75}%）</div>
              <div className="flex items-start gap-2"><span className="text-brand font-bold shrink-0">2.</span> 对接园区内传感器/芯片供应链企业，促进产业协作</div>
              <div className="flex items-start gap-2"><span className="text-brand font-bold shrink-0">3.</span> 关注企业人才需求，可推荐园区人才服务</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-card border border-line rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-2">
            <BarChart3 className="h-4 w-4 text-brand" /> 行业对标
          </h3>
    ...
    <div className="space-y-4">
      <div className="bg-surface-card border border-line rounded-lg p-5">
        <h3 className="text-sm font-semibold text-text-primary mb-4 flex items-center gap-2">
          <span className="w-1 h-4 bg-brand rounded-full" /> 工商信息
        </h3>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-surface-card border border-line rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-line-light flex items-center justify-between">
          <h3 className="text-sm font-semibold text-text-primary">历年财务数据</h3>
          <Button variant="text" size="sm" icon={<Sparkles className="h-3 w-3" />}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-surface-card border border-line rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3">资产结构</h3>
          <div className="space-y-2">
            {[
        ...
        <div className="bg-surface-card border border-line rounded-lg p-4">
          <h3 className="text-sm font-semibold text-text-primary mb-3">融资记录</h3>
          <div className="space-y-3">
            {[
...
function IPTab({ enterprise }: { enterprise: any }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          { label: '发明专利', value: 12, icon: Lightbulb, color: 'text-brand' },
          { label: '实用新型', value: 8, icon: Cpu, color: 'text-success' },
          { label: '外观设计', value: 3, icon: PieChart, color: 'text-brand' },
          { label: '软件著作权', value: 15, icon: BookOpen, color: 'text-warning' },
          { label: '商标', value: 5, icon: Award, color: 'text-error' },
        ].map((item, i) => (
          <div key={i} className="bg-surface-card border border-line rounded-lg p-4 text-center">
      ...
      <div className="bg-surface-card border border-line rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-line-light flex items-center justify-between">
          <h3 className="text-sm font-semibold text-text-primary">专利列表（Top 10）</h3>
          <Button variant="text" size="sm" icon={<Sparkles className="h-3 w-3" />}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-surface-card border border-line rounded-lg">
        <div className="px-4 py-3 border-b border-line-light flex items-center justify-between">
          <h3 className="text-sm font-semibold text-text-primary">核心团队</h3>
          <Button variant="text" size="sm" icon={<Sparkles className="h-3 w-3" />}
      ...
      {/* 股东结构 */}
      <div className="bg-surface-card border border-line rounded-lg p-4">
        <h3 className="text-sm font-semibold text-text-primary mb-4">股东结构</h3>
        <div className="space-y-2">
      ...
      {/* 员工构成 */}
      <div className="bg-surface-card border border-line rounded-lg p-4">
        <h3 className="text-sm font-semibold text-text-primary mb-3">员工构成</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
...
function VisitsTab({ visits, router }: { visits: any[]; router: any }) {
  ...
  return (
    <div className="space-y-3">
      {visits.map(v => (
        <div key={v.id} className="bg-surface-card border border-line rounded-lg p-4 hover:border-brand transition-colors cursor-pointer group"
          ...
          <div className="grid grid-cols-2 gap-4 bg-neutral-50 p-3 rounded text-sm">
            ...
            <div>
              <span className="text-text-muted block mb-1 text-xs">关键发现</span>
              <ul className="list-disc list-inside text-text-secondary space-y-0.5 text-xs">
  ...
  return (
    <div className="bg-surface-card border border-line rounded-lg overflow-hidden">
      <table className="w-full text-sm text-left">
        ...
        <tbody className="divide-y divide-line-light">
          {demands.map(d => (
            <tr key={d.id} className="hover:bg-surface-hover-row">
              <td className="px-4 py-3 text-text-primary font-medium">{d.demand_content}</td>
              <td className="px-4 py-3"><Tag variant="blue">{DEMAND_TYPE_LABELS[d.demand_type as DemandType]}</Tag></td>
              <td className="px-4 py-3 text-text-secondary text-xs">{d.assigned_department || '-'}</td>
...
function PolicyTab({ assessments }: { assessments: any[] }) {
  ...
  return (
    <div className="space-y-4">
      {assessments.map(a => (
        <div key={a.id} className="bg-surface-card border border-line rounded-lg p-4">
          <div className="flex items-center justify-between mb-4 border-b border-line-light pb-3">
            <div className="flex items-center gap-3">
              <h3 className="font-semibold text-text-primary">{a.policy_type}</h3>
              ...
              <span className="text-text-muted text-xs">得分 {a.grade_score}</span>
            </div>
            <div className="text-xs text-text-muted">更新于 {a.updated_at.split('T')[0]}</div>
          </div>
            ...
            {a.screening_details.map((d: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-2 bg-neutral-50 rounded border border-line-light">
                <div className="flex items-center gap-2">
                  ...
                  {d.result === 'pending' && <AlertCircle className="h-4 w-4 text-warning" />}
                  <span className="text-sm text-text-secondary">{d.rule_name}</span>
                </div>
