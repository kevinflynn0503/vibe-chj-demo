 ...
 *
 * Impeccable 重构：
 * - KPI 用 inline divider 排列，不用等尺寸卡片
...
import {
  ...
  ThumbsUp, ThumbsDown, ArrowDownCircle, Lightbulb,
  UserCheck, Send
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const pmAnalysis = pmProgress.map(pm => {
    ...
    return { ...pm, quality, qualityLabel, qualityColor, suggestion, overdue };
  });
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="page-container">
        <div className="flex items-center justify-between pt-sp-6 pb-sp-5">
          <div>
            <h1 className="text-xl font-semibold text-text-primary tracking-tight">管理看板</h1>
      ...
      <div className="page-container space-y-sp-7">
        {/* 子 Tab */}
        <div className="flex items-center gap-1 border-b border-line">
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={cn("relative px-3 py-2.5 text-sm font-medium transition-colors duration-normal ease-out-expo whitespace-nowrap cursor-pointer",
                activeTab === tab.key ? 'text-text-primary' : 'text-text-muted hover:text-text-secondary'
              )}>
              {tab.label}
              {tab.count !== undefined && tab.count > 0 && (
                <span className="ml-1.5 text-tag px-1.5 py-0.5 rounded-full bg-error text-text-inverse">{tab.count}</span>
              )}
              {activeTab === tab.key && (
        ...
        {/* ═══ 总览 ═══ */}
        {activeTab === 'overview' && (
          <div className="space-y-sp-7 stagger-in">
            {/* KPI — inline divider 排列，不用卡片 */}
            <div className="grid grid-cols-4 gap-0 divide-x divide-line py-1">
              {kpis.map((kpi, i) => (
                <div key={i} className={cn('flex flex-col', i === 0 ? 'pr-sp-5' : 'px-sp-5')}>
                  <div className="flex items-center gap-1.5 mb-2">
                    <kpi.icon className="h-3.5 w-3.5 text-text-muted" />
                    <span className="text-xs text-text-muted">{kpi.label}</span>
                  </div>
                  <span className="text-2xl font-semibold leading-none tabular-nums text-text-primary">{kpi.value}</span>
                  <span className={cn("text-xs font-medium mt-1.5 flex items-center gap-0.5",
            ...
            {/* 政策漏斗 — 水平 bar 替代居中大数字 */}
            <Card>
              <div className="flex items-center gap-2 mb-sp-4 p-4 pb-0">
              ...
              <div className="px-4 pb-4 space-y-2.5">
                {pipeline.map((s, i) => {
                  const maxVal = 17000;
                  const numVal = typeof s.value === 'string' ? parseInt(s.value.replace(',', '')) : s.value;
                  const pct = Math.max((numVal / maxVal) * 100, 2);
                  return (
                    <div key={i} className="flex items-center gap-3">
                      <span className="text-xs text-text-secondary w-16 shrink-0 text-right">{s.label}</span>
                      <div className="flex-1 h-6 bg-surface-primary rounded overflow-hidden relative">
                        <div
                          className={cn(
                            "h-full rounded transition-all duration-slow ease-out-expo",
                            i === 0 ? "bg-neutral-200" : "bg-brand-alpha-20"
                          )}
                          style={{ width: `${pct}%` }}
                        />
                        <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-medium text-text-primary tabular-nums">{s.value}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <Card className="p-0">
              ...
              <div className="divide-y divide-line-light">
                {pmAnalysis.map((pm, i) => (
                  <div key={i} className="px-4 py-3.5 flex items-center gap-4 transition-colors duration-normal ease-out-expo hover:bg-surface-hover-row">
                    <div className="w-8 h-8 rounded-full bg-surface-primary flex items-center justify-center text-xs font-semibold shrink-0 text-text-secondary">
            ...
            <Card className="p-0">
              ...
              <div className="divide-y divide-line-light">
                {[
                  ...
                  <div key={i} className="px-4 py-3 flex items-center gap-3 transition-colors duration-normal ease-out-expo hover:bg-surface-hover-row cursor-pointer group">
                    ...
                    <Button variant="link" size="sm" className="opacity-0 group-hover:opacity-100 transition-opacity duration-normal" onClick={() => {
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-3">
                {[
                  { v: inc.total_enterprises, l: '在孵企业' },
                  { v: inc.active_enterprises, l: '高活跃' },
                  { v: alerts.length, l: '异常预警' },
                  { v: inc.total_orders, l: '本月匹配' },
                  { v: inc.pending_orders, l: '待处理' },
                ].map((s, i) => (
                  <div key={i} className="text-center py-3 border border-line rounded-lg transition-all duration-normal ease-out-expo hover:shadow-card-hover hover:border-line-hover">
                    <div className="text-xl font-semibold text-text-primary tabular-nums">{s.v}</div>
                    <div className="text-tag text-text-muted mt-0.5">{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* ═══ 筛选审核 ═══ */}
        {activeTab === 'screening' && (
          <div className="space-y-sp-5 stagger-in">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-secondary">
                    AI 已完成第 3 轮高新筛选。以下是 A 级企业，请逐一审核。
                  </p>
                </div>
              </div>
            </div>
            ...
            {approvedCount > 0 && (
              <div className="border border-line rounded-lg p-3 flex items-center justify-between">
            ...
            <Card className="p-0 overflow-hidden">
              {assessments.filter(a => a.grade === 'A').map(a => {
                ...
                return (
                  <div key={a.id} className={cn("px-4 py-3.5 border-b border-line last:border-b-0 transition-all duration-normal ease-out-expo", review === 'excluded' && 'opacity-40')}>
                    <div className="flex items-center">
                      <div className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer"
                        onClick={() => router.push(`/policy/screening/${a.id}`)}>
                        <div className="w-8 h-8 rounded-lg bg-success-light text-success flex items-center justify-center text-xs font-semibold shrink-0">A</div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
        ...
        {activeTab === 'allocation' && (
          <div className="space-y-sp-5 stagger-in">
            <div className="bg-surface-card border border-line rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-text-secondary">
                    基于赛道专长、转化率和工作量，AI 为每个任务推荐了最佳负责人。
                  </p>
                </div>
            ...
            {/* PM 当前负荷 — inline divider 替代等尺寸卡片 */}
            <div className="grid grid-cols-4 gap-0 divide-x divide-line border border-line rounded-lg py-4">
              {pmProgress.map((pm, i) => (
                <div key={i} className={cn("px-sp-5")}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-semibold text-text-primary">{pm.name}</span>
                    <span className="text-xs text-text-muted tabular-nums">{(pm.conversion_rate * 100).toFixed(0)}%</span>
                  </div>
                  <div className="flex items-center gap-sp-4 text-xs tabular-nums">
                    <span className="text-text-secondary">分配 <strong className="text-text-primary">{pm.assigned}</strong></span>
                    <span className="text-text-secondary">走访 <strong className="text-brand">{pm.visited}</strong></span>
                    <span className="text-text-secondary">意愿 <strong className="text-success">{pm.willing}</strong></span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'team' && (
          <div className="space-y-sp-5 stagger-in">
            ...
            <Card className="p-0">
              ...
              <div className="divide-y divide-line-light">
                {pmAnalysis.map((pm, i) => (
                  <div key={i} className="px-4 py-sp-4">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-full bg-surface-primary flex items-center justify-center text-sm font-semibold shrink-0 text-text-secondary">
                        {pm.name.charAt(pm.name.length - 1)}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-4 gap-4 mb-3 tabular-nums">
                          <div><div className="text-tag text-text-muted">分配/完成</div><div className="text-sm font-semibold">{pm.visited}/{pm.assigned}</div></div>
                          <div><div className="text-tag text-text-muted">转化率</div><div className={cn("text-sm font-semibold", pm.qualityColor)}>{(pm.conversion_rate * 100).toFixed(0)}%</div></div>
                          <div><div className="text-tag text-text-muted">有意愿</div><div className="text-sm font-semibold text-success">{pm.willing}</div></div>
                          <div><div className="text-tag text-text-muted">待处理</div><div className={cn("text-sm font-semibold", pm.overdue > 3 ? 'text-error' : 'text-text-secondary')}>{pm.overdue}</div></div>
                        </div>
                        {pm.suggestion && (
                          <div className="flex items-start gap-2 p-2.5 bg-surface-primary border border-line rounded-lg">
