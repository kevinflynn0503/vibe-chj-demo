...
import {
  ...
  ChevronRight, Building2, Calendar, User, Plus,
  Users, Phone, Sparkles, Bot, Lightbulb
} from 'lucide-react';
...
export default function VisitWorkbench() {
  ...
  const prepareList = useMemo(() => {
    ...
    return enterprises.filter(e => !visitedIds.has(e.id)).slice(0, 6);
  }, [enterprises, records]);
    ...
    <div className="min-h-full">
      ...
      <div className="page-container">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-sp-6 pb-sp-5">
          <div>
            <h1 className="text-xl font-semibold text-text-primary tracking-tight">走访任务看板</h1>
            <p className="text-xs text-text-muted mt-1">准备 → 走访 → 确认 → 跟进 · {total} 项任务</p>
          </div>
          ...
          <Col title="确认阶段" count={confirmList.length} color="bg-warning">
            {confirmList.map(item => (
              <CardCompact key={item.id} onClick={() => router.push(`/visit/confirm/${item.id}`)}>
                ...
                {/* 去掉嵌套卡片 */}
                <div className="text-xs text-text-secondary mb-1.5">走访日期: {item.visit_date}</div>
                {item.key_findings && item.key_findings.length > 0 && (
                  <p className="text-xs text-warning line-clamp-2 mb-2">{item.key_findings[0]}</p>
                )}
                <div className="space-y-1 text-xs text-text-muted">
                ...
                <div className="mt-2.5 pt-2 border-t border-line-light flex items-center justify-between text-tag">
                  <span className="flex items-center gap-1 text-brand">
                    <Bot className="h-3 w-3" />AI 已提取，待确认
                  </span>
                </div>
              </CardCompact>
            ))}
          ...
          <Col title="跟进阶段" count={followList.length} color="bg-success">
            {followList.map((item, i) => (
              <CardCompact key={i} onClick={() => router.push(`/visit/${item.enterprise.id}/follow?demand=${item.demand.id}`)}>
                <div className="text-sm font-semibold text-text-primary mb-2">{item.enterprise.short_name ?? item.enterprise.name}</div>
                {/* 去掉嵌套卡片 */}
                <p className="text-xs text-text-secondary leading-relaxed line-clamp-2 mb-2">{item.demand.demand_content}</p>
                <div className="flex flex-wrap gap-1 mb-2">
...
function Col({ title, count, color, children }: { title: string; count: number; color: string; children: React.ReactNode }) {
  return (
    <div className="flex-1 min-w-[260px] max-w-[320px] flex flex-col h-full">
      <div className="flex items-center gap-2 mb-sp-3 px-1">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1 pb-4 stagger-in">{children}</div>
    </div>
  );
}
